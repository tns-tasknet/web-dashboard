...
<ul class="list rounded-box bg-base-100 shadow-md">
	<li class="p-4 pb-2 text-xs tracking-wide opacity-60">All users</li>
	...
	{#each data.users.users as user}
		<li class="list-row">
			...
			<div>
				<div>{user.name}</div>
				<div class="text-xs font-semibold opacity-60">{user.id}</div>
			</div>

			<form method="POST">
				<input name="userid" type="text" value={user.id} />
				<button class="btn btn-primary" type="submit">add to org</button>
			</form>
