<script lang="ts">
	...
	type SortKey = 'id' | 'title' | 'memberName' | 'status' | 'createdAt';
	type LoadParams = {
		q: string;
		index: number;
		size: number;
...
<div class="mb-4 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
	...
	<div class="flex w-full items-end justify-between gap-3 md:w-auto">
		...
		<a class="btn btn-primary md:self-end" href={`/dashboard/${org}/orders/create`}>
			Crear orden
		</a>
	</div>
</div>
			...
			<tr class="border-b">
				{#key `${sortKey}-${sortDir}`}
					...
					>
						...
						>
							<span>ID</span>
							<span class="opacity-70">{sortIcon('id')}</span>
						</button>
					</th>
					...
					>
						...
						>
							<span>Título</span>
							<span class="opacity-70">{sortIcon('title')}</span>
						</button>
					</th>
					...
					>
						...
						>
							...
							<span class="opacity-70">{sortIcon('memberName')}</span>
						</button>
					</th>
					...
					>
						...
						>
							<span>Estado</span>
							<span class="opacity-70">{sortIcon('status')}</span>
						</button>
					</th>
					...
					>
						...
						>
							...
							<span class="opacity-70">{sortIcon('createdAt')}</span>
						</button>
					</th>
				{/key}
			</tr>
			...
			<tbody>
				{#each reports as r}
					<tr
						...
						ondblclick={() =>
							goto(`/dashboard/${page.params.organizationSlug}/orders/${r.id}`)}
