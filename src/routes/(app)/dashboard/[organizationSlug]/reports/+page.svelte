<script lang="ts">
  ...
  async function loadReports(params: LoadParams) {
    ...
    try {
      const res = await fetch(`/api/v1/${org}/reports?${usp.toString()}`, { signal: controller.signal });
  ...
  function gotoPrev() { if (hasPrev) pageIndex -= 1; }
  function gotoNext() { if (hasNext) pageIndex += 1; }
  function gotoPage(i: number) { if (i >= 0 && i < totalPages) pageIndex = i; }
...
<div class="grid gap-3 md:grid-cols-4 mb-4">
  <div class="form-control md:col-span-2">
    ...
    <div class="mt-2 flex gap-2">
      <button class="btn btn-primary btn-sm" onclick={applySearch}>Buscar</button>
  ...
  <div class="form-control">
    ...
    <select id="size" class="select select-bordered" bind:value={pageSize}>
  ...
  <div class="md:col-span-1 flex items-end justify-end">
    <a class="btn btn-primary" href={`/dashboard/${org}/reports/create`}>
      Crear orden
    </a>
  </div>
</div>
...
<div class="overflow-x-auto rounded-box border border-base-300">
  <table class="table">
      ...
      <tbody>
        {#each Array.from({ length: Math.min(pageSize, 5) }) as _}
          <tr>
            ...
            <td class="text-right"><div class="skeleton h-8 w-16 ml-auto"></div></td>
          </tr>
        {/each}
      </tbody>
    {:else if errorMsg}
    ...
    {:else if reports.length === 0}
      <tbody>
        <tr>
          <td colspan="5"><div class="p-6 text-center opacity-70">Sin resultados.</div></td>
        </tr>
      </tbody>
    {:else}
      <tbody>
        {#each reports as r}
          <tr>
            ...
            <td class="text-right">
              <a class="btn btn-ghost btn-sm" href={`/dashboard/${page.params.organizationSlug}/reports/${r.id}`}>Ver</a>
            </td>
          </tr>
        {/each}
      </tbody>
