<script lang="ts">
	...
	function mockOpenPerDay(from: Date, to: Date) {
		...
		return ds.map((d, i) => {
			...
			const base =
				5 + Math.round(8 * Math.exp(-Math.pow((i - mid) / (ds.length / 5 || 1), 2)));
	...
	const kpi_created = $derived(
		openSeries.length > 0
			? Math.max(18, Math.round(openSeries.reduce((a, b) => a + (b.y > 0 ? 1 : 0), 0) * 1.2))
			: 0
	);
...
<div class="flex flex-1 items-center justify-between gap-4">
	<div class="flex flex-col">
		...
		<P class="mt-3 text-sm opacity-70">
			Rango seleccionado:
			{dateRange.from ? dateRange.from.toLocaleDateString() : '—'} – {dateRange.to
